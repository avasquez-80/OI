...
def obtener_info_fundamental_y_noticias(ticker_symbol):
    """Extrae datos fundamentales completos, sector, industria, earnings y noticias con yfinance."""
        ...
        try:
            ...
            if news_list:
                for item in news_list[:5]:
                    ...
                    if 'link' in item:
                        link = item['link']
                    elif 'content' in item and 'clickThroughUrl' in item['content']:
                        link = item['content']['clickThroughUrl'].get('url', '#')
                    elif 'content' in item and 'canonicalUrl' in item['content']:
                        link = item['content']['canonicalUrl'].get('url', '#')

                    if title:
                        noticias.append({
                            "title": title,
                            "publisher": publisher if publisher else "Medio Financiero",
                            "link": link
                        })
        except Exception:
            pass
...
def generar_analisis_robusto():
    ...
    <div class="container">
        ...
        <div class="card" style="background-color: #1f2937; border: 1px solid #374151; color: #f3f4f6;">
            <p style="margin-top: 0;">Análisis integral cruzando derivados, microestructura institucional (Dark/Lit), estrategia técnica de TradingView, sector y fundamentales para <strong>{ticker_ganador}</strong>.</p>
