...
def obtener_analisis_ia(ranking, ganador, stock_data):
    """Consulta a la API de Gemini para redactar el análisis institucional dinámico."""
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY"):
        ...
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            ...
            <ul>
                <li><strong>Concentración de Capital (Smart Money):</strong> El contrato líder (Strike ${ganador['Strike ($)']} {ganador['C/P']}) concentra el volumen principal de primas.</li>
                <li><strong>Lectura de Flujo:</strong> Máximo Heat Score registrado de {ganador['Max_Heat']}, reflejando alta agresividad institucional en las ejecuciones.</li>
    ...
    try:
        ...
        prompt = f"""
        Actúa como un analista cuantitativo senior de derivados financieros. 
        Analiza estos datos cuantitativos para el ticker {top_ticker}:
        - Contrato líder: Opción {cp} con strike {strike}, expiración {exp}.
        - Prima total acumulada: ${prima:,.2f}
        - Heat score máximo: {heat}
        - Sentimiento institucional: {ganador['Sentiment_Dominante']}
        
        Redacta un análisis institucional breve, profesional y directo (en español) con 3 viñetas estructuradas que incluyan:
        1. Concentración de capital y lectura de smart money.
        2. Perspectiva de riesgo/recompensa para este contrato.
        3. Una recomendación operativa clara de gestión de riesgo.
        ...
        return f"""
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🤖 Análisis Institucional Dinámico (IA Gemini)</h3>
            {response.text}
        </div>
        """
...
def generar_analisis_robusto():
    ...
    # 5. Extraer serie histórica del contrato ganador y fusionar
    contrato_hist = df[
    ...
    # 6. Generar Gráfica Avanzada de Paneles Múltiples
    ...
    # Obtener el análisis de IA de Google Gemini
    analisis_ia_html = obtener_analisis_ia(ranking, ganador, stock_data)
    ...
    <div class="container">
        <div class="header-banner">
            ...
            <p>Flujo Institucional de Opciones y Validación con Yahoo Finance (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Resumen Metodológico</h2>
        <div class="card">
            <p>Este informe integra el análisis de derivados y los datos de precio del subyacente (vía <strong>Yahoo Finance</strong>). Se evalúa a <strong>{ticker_ganador}</strong> en una ventana de <strong>10 semanas</strong> a partir del <strong>{fecha_analisis.strftime('%Y-%m-%d')}</strong>.</p>
        </div>
    ...
    # 8. Abrir en el navegador web de forma interactiva (archivo temporal)
    with tempfile.NamedTemporaryFile(suffix='.html', delete=False, mode='w', encoding='utf-8') as tmp_html:
        tmp_html.write(html_content)
        tmp_html_path = tmp_html.name

    print("\nAbriendo el informe interactivo con análisis de IA en tu navegador web...")
