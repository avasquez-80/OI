...
def procesar_dark_pools(ruta_dp):
    """Procesa de forma opcional el archivo CSV de Dark Pools y extrae métricas institucionales."""
    if not ruta_dp:
        return None
    try:
        ...
        print(f" -> Dark Pools cargados exitosamente: {num_impresiones} registros encontrados.")
        return resumen_dp
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dp=None):
    """Consulta a la API de Gemini para redactar el análisis institucional dinámico incluyendo Dark Pools (si existen)."""
    
    contexto_dp = ""
    if datos_dp:
        contexto_dp = f"""
        - Actividad institucional en Dark Pools del mes: {datos_dp['impresiones']} bloques registrados, sumando un monto total negociado de ${datos_dp['total_monto']:,.2f} con un volumen de {datos_dp['total_volumen']:,.0f} acciones.
        """
    else:
    ...
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY"):
        ...
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            ...
            <ul>
                ...
                {f"<li><strong>Actividad Dark Pools:</strong> {datos_dp['impresiones']} bloques institucionales detectados por ${datos_dp['total_monto']:,.2f}.</li>" if datos_dp else ""}
    ...
    try:
        ...
        2. Confluencia o divergencia con la actividad de Dark Pools (si aplica).
        ...
        return f"""
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🤖 Análisis Institucional Dinámico (IA Gemini + Dark Pools)</h3>
            {response.text}
        </div>
        """
    except Exception as e:
...
def generar_analisis_robusto():
    ...
    # 9. Obtener el análisis inteligente de la IA (Pasando datos_dp)
    ...
    <div class="container">
        <div class="header-banner">
            ...
            <p>Flujo Institucional de Opciones, Dark Pools y Validación con Yahoo Finance (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Resumen Metodológico</h2>
        <div class="card">
            <p>Este informe integra el análisis de derivados, los bloques institucionales y los datos de precio del subyacente (vía <strong>Yahoo Finance</strong>). Se evalúa a <strong>{ticker_ganador}</strong> en una ventana de <strong>10 semanas</strong> a partir del <strong>{fecha_analisis.strftime('%Y-%m-%d')}</strong>.</p>
        </div>
    ...
    print("\nAbriendo el informe interactivo con análisis de IA y Dark Pools en tu navegador web...")
