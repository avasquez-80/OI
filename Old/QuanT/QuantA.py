...
def generar_analisis_robusto():
    ...
    # 8. Generar Dashboard Gráfico Dinámico (2 o 3 Paneles según Dark Pools)
    print("\nGenerando dashboard gráfico avanzado y consultando a la IA para el informe...")
    
    tiene_dp = datos_dp is not None and not datos_dp['top_niveles'].empty
    
    if tiene_dp:
        fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(10, 11), dpi=300, sharex=False)
    else:
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 7), dpi=300, sharex=False)
    
    # Panel 1: Precio del Subyacente + Muros Dark Pools (si existen)
    if not stock_data.empty and 'Close' in stock_data.columns and not stock_data['Close'].isna().all():
        ...
        # Superponer muros institucionales como líneas horizontales
        if tiene_dp:
            top_muros = datos_dp['top_niveles'].head(4)
            for _, row in top_muros.iterrows():
                p_lvl = row['Price_Clean']
                amt_m = row['Total_Amount'] / 1e6
                ax1.axhline(y=p_lvl, color='#805ad5', linestyle='--', alpha=0.75, linewidth=1.5)
                ax1.text(tendencia_diaria['Trade Date'].iloc[0], p_lvl, f'  DP Wall: ${p_lvl:.2f} (${amt_m:.1f}M)', 
                         color='#6b46c1', fontsize=8, fontweight='bold', verticalalignment='bottom')

        ax1.legend(loc='upper left')
        ax1.set_title(f'Validación Cruzada: Acción Subyacente, Muros Dark Pool y Flujo ({ticker_ganador})', fontsize=11, fontweight='bold', pad=10)
    else:
    ...
    ax4 = ax2.twinx()
    color_prem = '#e53e3e'
    ax4.set_ylabel('Prima Diaria ($)', color=color_prem, fontweight='bold', fontsize=9)
    ax4.bar(tendencia_diaria['Trade Date'], tendencia_diaria['Daily_Premium'], color=color_prem, alpha=0.35, width=0.6, label='Prima Diaria')
    ax4.tick_params(axis='y', labelcolor=color_prem)
    ...
    # Panel 3: Gráfica de barras horizontales de Dark Pools (si el usuario cargó el archivo)
    if tiene_dp:
        top_niveles_plot = datos_dp['top_niveles'].head(5).sort_values(by='Total_Amount', ascending=True)
        precios = [f"${p:.2f}" for p in top_niveles_plot['Price_Clean']]
        montos_m = top_niveles_plot['Total_Amount'] / 1e6
        
        bars = ax3.barh(precios, montos_m, color='#805ad5', alpha=0.8, height=0.55)
        ax3.set_xlabel('Monto Negociado (Millones de $)', fontweight='bold', fontsize=9, color='#553c9a')
        ax3.set_ylabel('Nivel de Precio (Dark Pools)', fontweight='bold', fontsize=9, color='#553c9a')
        ax3.set_title('Top Muros de Acumulación Institucional (Dark Pool Levels)', fontsize=10, fontweight='bold', pad=8, color='#553c9a')
        ax3.grid(True, linestyle='--', alpha=0.4, axis='x')
        
        for bar in bars:
            width = bar.get_width()
            ax3.text(width + (width * 0.02), bar.get_y() + bar.get_height()/2, f'${width:.1f}M', 
    ...
    # 9. Obtener el análisis inteligente de la IA
    ...
    <div class="container">
        <div class="header-banner">
            ...
            <p>Flujo Institucional de Opciones, Niveles Dark Pool y Validación (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Resumen Metodológico</h2>
        <div class="card">
            <p>Este informe integra el análisis de derivados, los muros institucionales (Dark Pool Levels) y los datos del subyacente (vía <strong>Yahoo Finance</strong>). Se evalúa a <strong>{ticker_ganador}</strong> en una ventana de <strong>10 semanas</strong> a partir del <strong>{fecha_analisis.strftime('%Y-%m-%d')}</strong>.</p>
        </div>
        ...
        <h2>Dashboard Gráfico Institucional (Subyacente, Muros y Opciones)</h2>
        <div class="chart-container">
            <img src="data:image/png;base64,{chart_base64}" alt="Dashboard Institucional Integrado">
        </div>
    ...
    print("\nAbriendo el dashboard interactivo en tu navegador web predeterminado...")
