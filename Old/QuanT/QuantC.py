...
def obtener_info_fundamental_yf(ticker_symbol):
    """Extrae datos fundamentales y fecha de próximos earnings usando yfinance de forma segura."""
    ...
    try:
        ...
        # Intentar obtener la fecha de earnings
        ...
        try:
            ...
            # Fallback alternativo con earnings_dates si calendar falla
            try:
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    """Consulta a la IA cruzando derivados, subyacente, Dark/Lit Pools y contexto fundamental."""
    ...
    if info_fund:
        info_fund_str = f"""
        - Contexto Fundamental / Riesgo de Evento:
    ...
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY"):
        ...
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🔍 Análisis Institucional Estándar (Modular)</h3>
            <ul>
                <li><strong>Opciones (Smart Money):</strong> Contrato líder Strike ${ganador['Strike ($)']} {ganador['C/P']}.</li>
                {f"<li><strong>Próximo Earnings:</strong> {info_fund.get('proximo_earnings')}</li>" if info_fund else ""}
    ...
    try:
        ...
        2. Análisis cruzado entre Dark Pools, Lit Pools y el Strike ganador (${strike}).
        3. Evaluación de riesgo fundamental / evento (considerando la fecha del próximo reporte de ganancias frente al vencimiento del contrato).
        4. Una recomendación operativa clara de gestión de riesgo.
        ...
        return f"""
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🤖 Análisis Institucional Dinámico con Riesgo de Evento (IA Gemini)</h3>
            {response.text}
        </div>
        """
    except Exception as e:
...
def generar_analisis_robusto():
    ...
    # Obtener información fundamental y de earnings con yfinance
    ...
    if YFINANCE_DISPONIBLE:
        print(f"Descargando historial técnico de {ticker_ganador} desde Yahoo Finance...")
    ...
    # Generar Dashboard Gráfico Segregado
    print("Generando dashboard gráfico institucional...")
    ...
    # Panel 1: Precio Subyacente + Muros Dark Pool
    if not stock_data.empty and 'Close' in stock_data.columns and not stock_data['Close'].isna().all():
        ...
        ax1.text(0.5, 0.5, 'Datos de precio no disponibles', horizontalalignment='center', verticalalignment='center')
        ax1.set_title(f'Acción Subyacente ({ticker_ganador})', fontsize=11, fontweight='bold', pad=10)

    # Panel 2: Open Interest y Primas Diarias
    color_oi = '#3182ce'
    ...
    if tiene_dark and ax3 is not None:
        ...
        for bar in bars_d:
            w = bar.get_width()
            ax3.text(w + (w * 0.02), bar.get_y() + bar.get_height()/2, f'${w:.1f}M', ha='left', va='center', fontsize=8, fontweight='bold', color='#553c9a')

    # Panel 4: Lit Pools Segregados
    ...
    if tiene_lit and target_ax_lit is not None:
        ...
        for bar in bars_l:
            w = bar.get_width()
            target_ax_lit.text(w + (w * 0.02), bar.get_y() + bar.get_height()/2, f'${w:.1f}M', ha='left', va='center', fontsize=8, fontweight='bold', color='#b7791f')
    ...
    <div class="container">
        <div class="header-banner">
            <h1>Informe Cuantitativo con Riesgo de Evento</h1>
            <p>Flujo Institucional, Muros Segregados y Earnings Calendar (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Resumen Metodológico</h2>
        <div class="card">
            <p>Análisis integral de derivados, microestructura institucional (Dark/Lit Pools) y riesgo fundamental de earnings para <strong>{ticker_ganador}</strong>.</p>
        ...
        <h2>Dashboard Institucional Segregado</h2>
        <div class="chart-container">
            <img src="data:image/png;base64,{chart_base64}" alt="Dashboard Institucional">
        </div>
    ...
    html_content += f"""
        ...
        <div class="footer">
            Informe generado con integración de Google Gemini IA &bull; Referencia: {fecha_analisis.strftime('%Y-%m-%d')}
        </div>
    </div>
    </body>
    </html>
    ...
    print("\nAbriendo el informe interactivo con calendario de earnings en tu navegador...")
