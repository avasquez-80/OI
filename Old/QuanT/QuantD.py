...
def calcular_indicadores_tecnicos(stock_df):
    """Calcula EMAs, MACD y canales de soporte/resistencia dinámicos."""
    ...
    # Soportes y Resistencias Dinámicos (Canal Rolling 20 periodos)
    df['Support'] = df['Low'].rolling(window=20, min_periods=5).min()
    df['Resistance'] = df['High'].rolling(window=20, min_periods=5).max()
    
    return df

def obtener_info_fundamental_yf(ticker_symbol):
    """Extrae datos fundamentales y fecha de próximos earnings usando yfinance."""
    if not YFINANCE_DISPONIBLE:
        return {"proximo_earnings": "No disponible", "market_cap": "No disponible"}
    try:
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    """Consulta a la IA integrando derivados, microestructura, técnico (EMAs, MACD) y fundamentales."""
    ...
    if not stock_data.empty and 'Close' in stock_data.columns:
        ...
        p_act = ult['Close']
        ema20 = ult.get('EMA_20', p_act)
        ema50 = ult.get('EMA_50', p_act)
        macd_h = ult.get('MACD_Hist', 0)
        sop = ult.get('Support', p_act)
        res = ult.get('Resistance', p_act)
        
        tendencia_ema = "Alcista (Precio sobre EMA 20 y EMA 50)" if p_act > ema20 and ema20 > ema50 else ("Bajista" if p_act < ema20 else "Mixta/Consolidación")
        momentum_macd = "Positivo (Bullish)" if macd_h > 0 else "Negativo (Bearish)"
        
        contexto_tecnico = f"""
        - Análisis Técnico del Subyacente:
          * Precio Actual: ${p_act:.2f}
          * Tendencia EMA: {tendencia_ema} (EMA 20: ${ema20:.2f}, EMA 50: ${ema50:.2f})
          * Momentum MACD: Histograma {momentum_macd} ({macd_h:.4f})
          * Soporte Dinámico: ${sop:.2f} | Resistencia Dinámica: ${res:.2f}
        """

    info_fund_str = ""
    if info_fund:
        info_fund_str = f"""
        - Contexto Fundamental y Riesgo de Evento:
    ...
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY"):
        ...
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🔍 Análisis Institucional Estándar (Técnico + Cuantitativo)</h3>
            <ul>
    ...
    try:
        ...
        prompt = f"""
        Actúa como un analista cuantitativo senior de derivados, microestructura y análisis técnico institucional. 
        ...
        1. Lectura conjunta de la tendencia técnica (EMAs, MACD, Soportes/Resistencias) y el flujo de smart money en derivados.
        2. Confluencia entre los muros institucionales (Dark/Lit Pools) y los niveles técnicos clave del subyacente.
        3. Evaluación de riesgo fundamental / evento (reporte de earnings frente a la expiración del contrato).
        4. Una recomendación operativa clara de gestión de riesgo y ejecución.
        ...
        return f"""
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🤖 Análisis Institucional Dinámico Integral (IA Gemini)</h3>
            {response.text}
        </div>
        """
    except Exception as e:
...
def generar_analisis_robusto():
    ...
    print("Consultando información fundamental y calendario de earnings...")
    info_fund = obtener_info_fundamental_yf(ticker_ganador)
    
    stock_data = pd.DataFrame()
    if YFINANCE_DISPONIBLE:
        print(f"Descargando historial técnico y calculando indicadores para {ticker_ganador}...")
        inicio_hist = df['Trade Date'].min() - pd.Timedelta(days=60) # Suficiente historial para EMAs y canales
    ...
    if not stock_data.empty and 'Close' in stock_data.columns:
        tendencia_diaria = tendencia_diaria.merge(stock_data[['Trade Date', 'Close', 'Volume', 'EMA_20', 'EMA_50', 'MACD_Hist']], on='Trade Date', how='left')
    
    # Generar Dashboard Gráfico Integral (Precio + EMAs + MACD + Opciones + Dark/Lit)
    print("Generando dashboard gráfico institucional avanzado...")
    ...
    # Paneles: 1. Precio con EMAs y Soportes, 2. MACD, 3. Opciones (OI/Premium), 4. Dark Pools (si hay), 5. Lit Pools (si hay)
    num_panels = 3 # Base: Precio/EMAs, MACD, Opciones
    if tiene_dark: num_panels += 1
    if tiene_lit: num_panels += 1
    
    fig, axes = plt.subplots(num_panels, 1, figsize=(10, 3.5 * num_panels), dpi=300, sharex=False)
    
    # Asignación segura de ejes
    ax_price = axes[0]
    ax_macd = axes[1]
    ax_opt = axes[2]
    ax_dark = axes[3] if num_panels >= 4 and tiene_dark else None
    ax_lit = axes[4] if num_panels == 5 else (axes[3] if num_panels == 4 and not tiene_dark else None)

    # Panel 1: Precio Subyacente + EMAs + Muros Dark Pool
    if not stock_data.empty and 'Close' in stock_data.columns and not stock_data['Close'].isna().all():
        ax_price.plot(stock_data['Trade Date'], stock_data['Close'], color='#2b6cb0', linewidth=1.8, label=f'Cierre {ticker_ganador}')
        if 'EMA_20' in stock_data.columns:
            ax_price.plot(stock_data['Trade Date'], stock_data['EMA_20'], color='#d69e2e', linestyle='--', linewidth=1.2, label='EMA 20')
        if 'EMA_50' in stock_data.columns:
            ax_price.plot(stock_data['Trade Date'], stock_data['EMA_50'], color='#dd6b20', linestyle=':', linewidth=1.2, label='EMA 50')
        if 'Support' in stock_data.columns:
            ax_price.plot(stock_data['Trade Date'], stock_data['Support'], color='#38a169', alpha=0.5, linestyle='-.', label='Soporte Dinámico')
            
        ax_price.set_ylabel('Precio Subyacente ($)', color='#2b6cb0', fontweight='bold', fontsize=9)
        ...
        ax_price.set_title(f'Tendencia Técnica, EMAs y Muros Institucionales ({ticker_ganador})', fontsize=10, fontweight='bold', pad=8)
    else:
        ax_price.text(0.5, 0.5, 'Datos técnicos no disponibles', horizontalalignment='center', verticalalignment='center')
        ax_price.set_title(f'Acción Subyacente ({ticker_ganador})', fontsize=10, fontweight='bold', pad=8)

    # Panel 2: Oscilador MACD
    if not stock_data.empty and 'MACD_Hist' in stock_data.columns:
        ...
        ax_macd.set_title('Oscilador MACD (Momentum)', fontsize=10, fontweight='bold', pad=8)

    # Panel 3: Open Interest y Primas Diarias de Opciones
    color_oi = '#3182ce'
    ...
    <head>
    <meta charset="UTF-8">
    <title>Informe Cuantitativo Integral con IA</title>
    <style>
    ...
    <div class="container">
        <div class="header-banner">
            <h1>Informe Cuantitativo con Análisis Técnico e IA</h1>
            <p>EMAs, MACD, Muros Institucionales y Calendario de Earnings (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Resumen Metodológico</h2>
        <div class="card">
            <p>Análisis integral de derivados, microestructura institucional (Dark/Lit Pools), indicadores técnicos y riesgo fundamental para <strong>{ticker_ganador}</strong>.</p>
        ...
        <h2>Dashboard Técnico e Institucional Avanzado</h2>
        <div class="chart-container">
            <img src="data:image/png;base64,{chart_base64}" alt="Dashboard Técnico Avanzado">
        </div>
    ...
    html_content += f"""
        ...
        <div class="footer">
            Informe generado con análisis técnico y Google Gemini IA &bull; Referencia: {fecha_analisis.strftime('%Y-%m-%d')}
        </div>
    </div>
    </body>
    </html>
    ...
    print("\nAbriendo el dashboard interactivo técnico con análisis de IA en tu navegador...")
