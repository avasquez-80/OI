...
def calcular_indicadores_tecnicos(stock_df):
    ...
    # EMAs exactas de tu configuración de TV (9 y 21)
    ...
    # Color de Volumen estilo TV (Verde si cierra >= abre o >= anterior cierre, Rojo si baja)
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    """Consulta a la IA integrando derivados, microestructura, estrategia EMA 9/21, MACD y fundamentales."""
    ...
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY"):
        ...
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🔍 Análisis Institucional Estándar (EMA 9/21 + Opciones)</h3>
            <ul>
                <li><strong>Opciones (Smart Money):</strong> Contrato líder Strike ${ganador['Strike ($)']} {ganador['C/P']}.</li>
                <li><strong>Recomendación:</strong> Vigilar cruce de EMAs alineado al vencimiento del {ganador['Expiry'].strftime('%Y-%m-%d')}.</li>
            </ul>
        </div>
        """
    ...
    try:
        ...
        prompt = f"""
        Actúa como un analista cuantitativo senior de derivados, microestructura y trading técnico (especialista en cruces EMA 9/21, MACD y Volumen). 
        ...
        3. Evaluación de riesgo fundamental / evento (reporte de earnings frente al vencimiento del contrato).
        4. Una recomendación operativa clara de entrada/salida basada en la confirmación técnica y de derivados.
        ...
        return f"""
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🤖 Análisis Institucional Integral (Estrategia TV + IA Gemini)</h3>
            {response.text}
        </div>
        """
    except Exception as e:
...
def generar_analisis_robusto():
    ...
    if YFINANCE_DISPONIBLE:
        print(f"Descargando historial técnico diario y calculando EMAs 9/21 y MACD para {ticker_ganador}...")
    ...
    # Generar Dashboard Gráfico Exacto (Precio+EMA9/21, Volumen, MACD, Opciones, Dark, Lit)
    ...
    num_panels = 4 # 1. Precio con EMA 9/21, 2. Volumen, 3. MACD, 4. Opciones (OI/Premium)
    ...
    # Panel 1: Precio + EMA 9 (Azul) + EMA 21 (Rojo) + Muros Dark
    if not stock_data.empty and 'Close' in stock_data.columns and not stock_data['Close'].isna().all():
        ...
        if 'EMA_9' in stock_data.columns:
            ax_price.plot(stock_data['Trade Date'], stock_data['EMA_9'], color='#3182ce', linewidth=1.8, label='EMA 9 (Rápida)')
        if 'EMA_21' in stock_data.columns:
            ax_price.plot(stock_data['Trade Date'], stock_data['EMA_21'], color='#e53e3e', linewidth=1.8, label='EMA 21 (Lenta)')
    ...
    # Panel 2: Volumen Diario Estilo TV
    ...
    # Panel 3: Oscilador MACD (12, 26, 9)
    ...
    # Panel 4: Open Interest y Primas Diarias de Opciones
    color_oi = '#3182ce'
    ...
    <div class="container">
        <div class="header-banner">
            <h1>Informe Cuantitativo Integrado (Estrategia TV + IA)</h1>
            <p>EMA 9/21, Volumen, MACD, Muros Institucionales y Earnings (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Resumen Metodológico</h2>
        <div class="card">
            <p>Análisis integral de derivados, microestructura institucional (Dark/Lit Pools), estrategia técnica EMA 9/21 y riesgo fundamental para <strong>{ticker_ganador}</strong>.</p>
    ...
    html_content += f"""
        ...
        <div class="footer">
            Informe generado con tu estrategia de TradingView y Google Gemini IA &bull; Referencia: {fecha_analisis.strftime('%Y-%m-%d')}
        </div>
    </div>
    </body>
    </html>
    ...
    print("\nAbriendo el informe interactivo con tu configuración de TradingView...")
