...
def obtener_info_fundamental_y_noticias(ticker_symbol):
    """Extrae datos fundamentales, fecha de earnings y noticias con enlaces usando yfinance."""
    if not YFINANCE_DISPONIBLE:
        return {"proximo_earnings": "No disponible", "market_cap": "No disponible", "noticias": []}
    try:
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    ...
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY"):
        ...
        """, "<p>Resumen de noticias no disponible sin IA activa.</p>"

    def llamar_con_reintentos(client, model, prompt, max_intentos=3):
        """Función auxiliar para reintentar la llamada si hay error 503 por alta demanda."""
    ...
    try:
        ...
        Devuélvelo en formato HTML limpio utilizando etiquetas <p> y <strong>.
        """
        ...
        analisis_html = f"""
        <div class="card" style="border-left: 4px solid #3182ce; background-color: #f7fafc;">
            <h3 style="margin-top: 0; color: #1a365d;">🤖 Análisis Institucional Integral (IA Gemini)</h3>
            {response_main.text}
        </div>
        """
        resumen_noticias_html = f"""
        <div class="card" style="border-left: 4px solid #d69e2e; background-color: #fffaf0;">
            <h3 style="margin-top: 0; color: #975a16;">📰 Síntesis Inteligente de Noticias y Sentimiento</h3>
            {response_news.text}
        </div>
        """
        ...
        fallback_main = f"<p><em>Análisis de IA temporalmente no disponible por saturación del servidor: {e}</em></p>"
        fallback_news = "<p>Resumen de noticias no disponible temporalmente.</p>"
...
def generar_analisis_robusto():
    ...
    <div class="container">
        ...
        <div class="card">
            <p>Análisis integral cruzando derivados, microestructura institucional (Dark/Lit), estrategia técnica de TradingView y síntesis de noticias recientes para <strong>{ticker_ganador}</strong>.</p>
