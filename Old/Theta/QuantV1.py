...
def preguntar_modo_api():
    ...
    root = tk.Tk()
    root.withdraw()  # Oculta la ventana principal de Tkinter para dejar solo el popup
    
    # Muestra la ventana emergente con botones Sí/No
    respuesta = messagebox.askyesno(
        "Selección de Motor de IA",
        "¿Deseas utilizar el MODO PAGO?\n\n"
        "• SÍ: Usa la API de pago (Modelo Pro, sin límites, análisis profundo).\n"
        "• NO: Usa la API gratuita (Modelo Flash, límite diario)."
    )
    root.destroy()
    return respuesta

# Ejecutar el selector visual
MODO_PAGO = preguntar_modo_api()

# Asignar la llave y modelo correspondiente según tu elección en el popup
if MODO_PAGO:
...
def obtener_info_fundamental_y_noticias(ticker_symbol):
    """Extrae el nombre oficial, datos fundamentales, sector, industria, earnings y noticias con yfinance."""
    if not YFINANCE_DISPONIBLE:
        return {
            "long_name": ticker_symbol, "proximo_earnings": "No disponible", "market_cap": "No disponible", 
    ...
    try:
        ...
        # Obtener el nombre oficial completo de la compañía
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    ...
    if not GEMINI_DISPONIBLE or not os.environ.get("GEMINI_API_KEY") or not os.environ.get("GEMINI_API_KEY_FREE"):
        return f"""
    ...
    def llamar_con_reintentos(client, model, prompt, max_intentos=3):
            ...
            try:
                ...
                if "503" in str(e) or "UNAVAILABLE" in str(e):
                    if intento < max_intentos - 1:
                        ...
                        print(f" -> Servidores de IA ocupados (503). Reintentando en {tiempo_espera} segundos (Intento {intento+2}/{max_intentos})...")
    ...
    try:
        ...
        prompt_principal = f"""
        Actúa como un director de riesgo cuantitativo y analista senior de derivados institucionales. 
        Realiza una disección exhaustiva y de nivel directivo para el ticker {top_ticker} basada en los siguientes datos cuantitativos:
        ...
        Redacta un informe ejecutivo institucional riguroso, técnico y sin rodeos (en español) estructurado en 4 apartados profundos:
        1. **Confluencia Técnica y de Derivados:** Evalúa la interacción matemática entre el cruce de las EMAs (9/21), el histograma MACD y la dominancia del contrato de opciones líder frente a la estructura de precios actual.
        2. **Microestructura y Elasticidad de Muros (Dark/Lit Pools):** Analiza el volumen acumulado en los bloques institucionales y calcula la resistencia o soporte operativo que representan frente a un movimiento direccional.
        3. **Calibración de Riesgo Fundamental y Eventos:** Cruza los datos de PER, Beta y proximidad de earnings con la fecha de expiración del contrato para determinar el riesgo de compresión de volatilidad o saltos por catalizadores.
        4. **Estrategia Operativa y Gestión de Riesgo Asimétrica:** Define escenarios cuantitativos claros (zona de acumulación óptima, invalidación técnica estricta y objetivos de toma de ganancias con una relación riesgo/beneficio superior a 1:3).
        
        Usa etiquetas <ul> y <li> limpias con formato profesional. No agregues bloques de código markdown alrededor.
        """
        
        print("Consultando a Gemini (Análisis Institucional)...")
        response_main = llamar_con_reintentos(client, 'gemini-3.6-flash', prompt_principal)
        texto_main = limpiar_markdown_ia(response_main.text)
        
        prompt_noticias = f"""
        Actúa como un analista de noticias financieras. Con base en los títulos de noticias recientes para el ticker {top_ticker} y sabiendo que nuestro análisis cuantitativo de opciones indica un sentimiento '{sentimiento_opciones}':
        Redacta un **resumen ejecutivo conciso y preciso en español** (máximo 3 párrafos cortos) de los eventos noticiosos clave. 
        Al final del resumen, indica claramente si el sentimiento de las noticias **confirma la tendencia alcista/bajista** de nuestro análisis cuantitativo o si, por el contrario, **indica divergencias o riesgos ocultos** que debamos considerar.
        Devuélvelo en formato HTML limpio utilizando etiquetas <p> y <strong>. No agregues bloques de código markdown alrededor.
        """
        
        print("Consultando a Gemini (Síntesis de Noticias)...")
        response_news = llamar_con_reintentos(client, 'gemini-3.6-flash', prompt_noticias)
...
def generar_analisis_robusto():
    ...
    print("Consultando información fundamental, calendario de earnings y noticias con yfinance...")
    ...
    if noticias_arr:
        ...
        for n in noticias_arr:
            links_html += f"<li style='margin-bottom: 6px;'><a href='{n['link']}' target='_blank' style='color: #2b6cb0; text-decoration: none; font-weight: 500;'>{n['title']}</a> <span style='color: #718096; font-size: 8.5pt;'>({n['publisher']})</span></li>"
        links_html += "</ul>"
    else:
        links_html = "<p style='color: #718096; margin: 0;'>No hay enlaces de noticias disponibles para este ticker.</p>"
    ...
    <div class="container">
        ...
        <h2>Resumen Metodológico y Radiografía Fundamental</h2>
        <div class="card" style="background-color: #1f2937; border: 1px solid #374151; color: #f3f4f6;">
            <p style="margin-top: 0; font-size: 11pt;">
                Activo Analizado: <strong style="color: #60a5fa; font-size: 12pt;">{info_fund.get('long_name')} ({ticker_ganador})</strong>
            </p>
            <p style="margin-bottom: 0; color: #9ca3af; font-size: 9.5pt;">Análisis integral cruzando derivados, microestructura institucional (Dark/Lit), estrategia técnica y sectorial.</p>
            ...
            <table style="width: 100%; font-size: 9.5pt; margin: 0; background: transparent;">
                <tr>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Sector:</strong> <span style="color: #f3f4f6;">{info_fund.get('sector')}</span></td>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Industria:</strong> <span style="color: #f3f4f6;">{info_fund.get('industry')}</span></td>
                </tr>
                <tr>
