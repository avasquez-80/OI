...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    ...
    if GEMINI_DISPONIBLE and API_KEY and API_KEY not in ["TU_CLAVE_DE_PAGO_AQUI", "TU_CLAVE_GRATUITA_AQUI"] and info_fund:
        ...
        if sum_orig and sum_orig != "No disponible" and sum_orig != "Resumen corporativo no disponible temporalmente.":
            try:
                ...
                prompt_trad = f"Traduce el siguiente resumen corporativo financiero al español de forma profesional, fluida y natural, manteniendo la precisión técnica:\n\n{sum_orig}"
    ...
    try:
        ...
        5. **Perspectivas a Futuro y Estrategia Asimétrica:** Qué se espera de la empresa de cara a sus próximos reportes (earnings) y proyecciones, definiendo zonas de operación con relación riesgo/beneficio superior a 1:3.
...
def generar_analisis_robusto():
    ...
    <div class="container">
        ...
        <div class="card" style="background-color: #1f2937; border: 1px solid #374151; border-left: 4px solid #3b82f6; padding: 15px 20px; color: #f3f4f6;">
            <p style="margin-top: 0; margin-bottom: 6px; color: #f3f4f6;"><strong>Opción Líder:</strong> Call con Strike <strong style="color: #60a5fa;">${strike_ganador}</strong> y Vencimiento <span style="color: #f3f4f6;">{exp_ganadora.strftime('%Y-%m-%d')}</span></p>
