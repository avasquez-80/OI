...
def importar_o_actualizar_historial(df_nuevo_lote):
    """
    Fusiona de forma segura nuevos datos (históricos o diarios) en la base de datos central.
    Protege los datos de otros tickers y actualiza inteligentemente las filas coincidentes 
    utilizando una clave compuesta [Trade Date, Ticker, Expiry, Strike ($), C/P].
    """
    ...
    print(f"✅ Base de datos actualizada con éxito. Total de registros consolidados en la BD: {len(df_final)}")

# --- SELECTOR GRÁFICO DE MOTOR IA (POPUP) ---
def preguntar_modo_api():
    """Muestra un popup al inicio para elegir entre modo pago o gratuito."""
    try:
        root = tk.Tk()
        root.withdraw()
        ...
        respuesta = messagebox.askyesno(
            ...
            "¿Deseas utilizar el MODO PAGO?\n\n"
            "• SÍ: Usa la API de pago (Modelo Pro, sin límites, análisis profundo).\n"
            "• NO: Usa la API gratuita (Modelo Flash, límite diario de 20 peticiones)."
        )
        root.destroy()
        return respuesta
    except Exception:
...
# Inicializar selección de API y Modelo
MODO_PAGO = preguntar_modo_api()

if MODO_PAGO:
    API_KEY = os.environ.get("GEMINI_API_KEY")
    MODELO_IA = "gemini-3.1-pro-preview"
    print("🚀 Modo Seleccionado: PAGO (Análisis Institucional Pro)")
else:
    API_KEY = os.environ.get("GEMINI_API_KEY_FREE")
    MODELO_IA = "gemini-3.6-flash"
    print("⚡ Modo Seleccionado: GRATUITO (Pruebas Rápidas Flash)")
...
def obtener_info_fundamental_y_noticias(ticker_symbol):
    """Extrae el nombre oficial, logo, resumen de negocio, datos fundamentales, sector, industria, earnings y noticias con yfinance."""
    if not YFINANCE_DISPONIBLE:
        return {
            "long_name": ticker_symbol, "logo_url": "", "summary": "No disponible",
            "proximo_earnings": "No disponible", "market_cap": "No disponible", 
            "sector": "No disponible", "industry": "No disponible", 
            "pe": "N/D", "beta": "N/D", "dividend": "N/D", 
            "high_52": "N/D", "low_52": "N/D", "noticias": []
        }
    try:
        ...
        summary = info.get('longBusinessSummary', 'Resumen corporativo no disponible temporalmente.')
        
        market_cap = info.get('marketCap', 0)
        market_cap_str = f"${market_cap:,.0f}" if market_cap else "No disponible"
        
        sector = info.get('sector', 'No especificado')
        industry = info.get('industry', 'No especificado')
        
        pe = info.get('trailingPE', 'N/D')
        pe_str = f"{pe:.2f}" if isinstance(pe, (int, float)) else "N/D"
        
        beta = info.get('beta', 'N/D')
        beta_str = f"{beta:.2f}" if isinstance(beta, (int, float)) else "N/D"
        
        div = info.get('dividendYield', None)
        div_str = f"{div * 100:.2f}%" if isinstance(div, (int, float)) else "N/D"
        
        high_52 = info.get('fiftyTwoWeekHigh', 'N/D')
        high_52_str = f"${high_52:.2f}" if isinstance(high_52, (int, float)) else "N/D"
        
        low_52 = info.get('fiftyTwoWeekLow', 'N/D')
        low_52_str = f"${low_52:.2f}" if isinstance(low_52, (int, float)) else "N/D"
        ...
        try:
            ...
            if news_list:
                for item in news_list[:5]:
                    ...
                    link = item.get('link', '#')
                    if title:
                        noticias.append({"title": title, "publisher": publisher or "Medio Financiero", "link": link})
        except Exception:
            pass
        ...
        return {
            "long_name": ticker_symbol, "logo_url": "", "summary": "No disponible",
            "proximo_earnings": "No disponible", "market_cap": "No disponible", 
            "sector": "N/D", "industry": "N/D", "pe": "N/D", "beta": "N/D", 
            "dividend": "N/D", "high_52": "N/D", "low_52": "N/D", "noticias": []
        }

def limpiar_markdown_ia(texto):
    """Limpia las etiquetas de bloque de código markdown si la IA las incluye."""
    if not texto:
        return ""
    texto = texto.strip()
...
def obtener_analisis_ia(ranking, ganador, stock_data, datos_dark=None, datos_lit=None, info_fund=None):
    """Consulta a la IA integrando derivados, microestructura, técnica EMA 9/21, fundamentales y síntesis de noticias con reintentos."""
    
    # Traducir el resumen de negocio de yfinance al español usando la IA si está disponible
    if GEMINI_DISPONIBLE and API_KEY and API_KEY not in ["TU_CLAVE_DE_PAGO_AQUI", "TU_CLAVE_GRATUITA_AQUI"] and info_fund:
        sum_orig = info_fund.get('summary', '')
        if sum_orig and sum_orig != "No disponible" and sum_orig != "Resumen corporativo no disponible temporalmente.":
            try:
                temp_client = genai.Client(api_key=API_KEY)
                prompt_trad = f"Traduce y resume el siguiente texto corporativo financiero al español con precisión técnica y tono profesional. Ve directamente al grano: entrega únicamente el texto traducido y condensado, sin introducciones, saludos ni comentarios previos:\n\n{sum_orig}"
                res_trad = temp_client.models.generate_content(model=MODELO_IA, contents=prompt_trad)
                info_fund['summary'] = limpiar_markdown_ia(res_trad.text)
            except Exception:
                pass
    ...
    info_fund_str = ""
    if info_fund:
        titulares_titulos = [n['title'] for n in info_fund.get('noticias', [])]
        titulares_texto = "\n".join(titulares_titulos) if titulares_titulos else "Sin noticias recientes destacadas."
        info_fund_str = f"""
        - Contexto Fundamental, Negocio y Noticias:
          * Empresa y Negocio: {info_fund.get('summary', 'N/D')[:700]}...
          ...
          * Títulos de Noticias Recientes:
          {titulares_texto}
        """

    if not GEMINI_DISPONIBLE or not API_KEY or API_KEY in ["TU_CLAVE_DE_PAGO_AQUI", "TU_CLAVE_GRATUITA_AQUI"]:
        return f"""
        <div class="card" style="border-left: 4px solid #3b82f6; background-color: #1f2937; color: #f3f4f6;">
            <h3 style="margin-top: 0; color: #60a5fa;">🔍 Análisis Institucional Estándar</h3>
            <ul>
        ...
        """, "<p style='color: #f3f4f6;'>Resumen de noticias no disponible sin IA activa.</p>"

    def llamar_con_reintentos(client_obj, model, prompt, max_intentos=3):
        for intento in range(max_intentos):
            try:
                res = client_obj.models.generate_content(model=model, contents=prompt)
                return res
            except Exception as e:
                if "503" in str(e) or "UNAVAILABLE" in str(e) or "429" in str(e):
                    if intento < max_intentos - 1:
                        tiempo_espera = 3 * (intento + 1)
                        print(f" -> Servidores de IA ocupados/límite. Reintentando en {tiempo_espera} segundos (Intento {intento+2}/{max_intentos})...")
    ...
    try:
        ...
        sentimiento_opciones = ganador['Sentiment_Dominante']
        nombre_empresa = info_fund.get('long_name', top_ticker) if info_fund else top_ticker
        
        prompt_principal = f"""
        Actúa como un director de riesgo cuantitativo y analista financiero senior. 
        Analiza de forma exhaustiva el activo {nombre_empresa} ({top_ticker}) considerando su perfil de negocio, contexto fundamental y cuantitativo:
        - Contrato líder de opciones: Opción {cp} con strike ${strike}, expiración {exp} (Sentimiento Opciones: {sentimiento_opciones}).
        - Prima total negociada: ${prima:,.2f} | Heat Score Máx: {heat}
        {contexto_tecnico}
        {contexto_pools}
        {info_fund_str}
        
        REGLA CRÍTICA DE FORMATO: No incluyas membretes, títulos de memorándum, introducciones ni presentaciones de ningún tipo. Comienza de inmediato con el desglose técnico y fundamental.
        
        Desarrolla un informe institucional estructurado estrictamente en 5 apartados con viñetas HTML (<ul> y <li>), redactado de forma clara y didáctica para que cualquier inversor pueda comprenderlo con facilidad:
        1. **Perfil y Tesis del Negocio:** Qué hace exactamente la empresa, su posición competitiva y cómo ha evolucionado su crecimiento o decaimiento financiero recientemente.
        2. **Catalizadores y Contexto de la Tendencia:** Por qué el activo se está moviendo de esta forma en el corto plazo, conectando las noticias recientes con el sentimiento del mercado.
        3. **Confluencia Técnica y de Derivados:** Interacción matemática entre el cruce de las EMAs (9/21), el histograma MACD y la dominancia del contrato de opciones líder.
        4. **Microestructura y Muros (Dark/Lit Pools):** Volumen institucional acumulado en los bloques y su función operativa como soporte o resistencia clave.
        5. **Perspectivas a Futuro, Gestión de Riesgo y Veredicto Táctico:** Análisis crítico de la asimetría estadística. Evalúa si el entorno presenta señales mixtas o baja convicción para recomendar explícitamente **no hacer nada** y preservar capital. Si existe una ventaja real, establece con precisión quirúrgica los *triggers* técnicos obligatorios (cruce de EMAs, cambio de estructura, retesteo con volumen) y una relación riesgo/beneficio superior a 1:3 antes de validar cualquier operación.
        
        Usa únicamente etiquetas <ul> y <li> limpias. No agregues bloques de código markdown alrededor.
        """
        
        print(f"Consultando a Gemini [{MODELO_IA}] (Análisis Institucional)...")
        response_main = llamar_con_reintentos(client, MODELO_IA, prompt_principal)
        texto_main = limpiar_markdown_ia(response_main.text)
        
        prompt_noticias = f"""
        A partir de los títulos de noticias recientes para el ticker {top_ticker} y sabiendo que nuestro análisis cuantitativo de opciones indica un sentimiento '{sentimiento_opciones}':
        Redacta un **resumen ejecutivo conciso y preciso en español** (máximo 3 párrafos cortos) de los eventos noticiosos clave. 
        Al final del resumen, indica claramente si el sentimiento de las noticias **confirma la tendencia** de nuestro análisis cuantitativo o si, por el contrario, **indica divergencias o riesgos ocultos** que debamos considerar.
        Devuélvelo en formato HTML limpio utilizando etiquetas <p> y <strong>. No agregues bloques de código markdown alrededor.
        """
        
        print(f"Consultando a Gemini [{MODELO_IA}] (Síntesis de Noticias)...")
        response_news = llamar_con_reintentos(client, MODELO_IA, prompt_noticias)
        texto_news = limpiar_markdown_ia(response_news.text)
        
        analisis_html = f"""
        <div class="card" style="border-left: 4px solid #3b82f6; background-color: #1f2937; color: #f3f4f6;">
            <h3 style="margin-top: 0; color: #60a5fa;">🤖 Análisis Institucional Integral (IA Gemini)</h3>
            <div style="color: #f3f4f6; font-size: 10pt;">
                {texto_main}
            </div>
        </div>
        """
        resumen_noticias_html = f"""
        <div class="card" style="border-left: 4px solid #f59e0b; background-color: #1f2937; color: #f3f4f6;">
            <h3 style="margin-top: 0; color: #fbbf24;">📰 Síntesis Inteligente de Noticias y Sentimiento</h3>
            <div style="color: #f3f4f6; font-size: 10pt;">
                {texto_news}
            </div>
        </div>
        """
        return analisis_html, resumen_noticias_html
    except Exception as e:
        print(f"Advertencia: No se pudo completar la consulta a la IA tras varios intentos: {e}")
        fallback_main = f"<p style='color: #f3f4f6;'><em>Análisis de IA temporalmente no disponible por saturación del servidor: {e}</em></p>"
        fallback_news = "<p style='color: #f3f4f6;'>Resumen de noticias no disponible temporalmente.</p>"
        return fallback_main, fallback_news
...
def generar_analisis_robusto():
    ...
    # Asegurar que solo analicemos contratos cuya expiración sea VIGENTE (mayor o igual al día de hoy)
    # y que se encuentren dentro de la ventana de proyección de hasta 10 semanas.
    fecha_analisis = df['Trade Date'].max()
    fecha_actual = pd.Timestamp.now().normalize()
    semanas_proyeccion = 10
    fecha_limite = fecha_actual + pd.Timedelta(weeks=semanas_proyeccion)
    
    df_window = df[(df['Expiry'] >= fecha_actual) & (df['Expiry'] <= fecha_limite)].copy()
    ...
    print("Consultando información fundamental, perfil de negocio y noticias con yfinance...")
    ...
    contrato_hist = df[
        ...
        (df['C/P'] == cp_ganador)
    ].sort_values(by=['Trade Date', 'Time']) if 'Time' in df.columns else df[
        (df['Ticker'] == ticker_ganador) &
        (df['Expiry'] == exp_ganadora) &
        (df['Strike ($)'] == strike_ganador) &
        (df['C/P'] == cp_ganador)
    ...
    tendencia_diaria = contrato_hist.groupby('Trade Date').agg(
        ...
        Volume=('Size', 'sum') # Asegurar el volumen negociado del contrato en el día
    ).reset_index()
    
    if not stock_data.empty and 'Close' in stock_data.columns:
        tendencia_diaria = tendencia_diaria.merge(stock_data[['Trade Date', 'Close', 'Volume', 'EMA_9', 'EMA_21', 'MACD_Hist']], on='Trade Date', how='left', suffixes=('', '_stock'))

    # --- INYECCIÓN AUTOMÁTICA A LA BASE DE DATOS DE SEGUIMIENTO ---
    if not tendencia_diaria.empty:
        df_para_db = pd.DataFrame({
            'Trade Date': pd.to_datetime(tendencia_diaria['Trade Date']).dt.strftime('%Y-%m-%d'),
            'Ticker': ticker_ganador.upper(),
            'Expiry': exp_ganadora.strftime('%Y-%m-%d'),
            'Strike ($)': float(strike_ganador),
            'C/P': cp_ganador.upper(),
            'Open Interest': tendencia_diaria['Open_Interest'].fillna(0).astype(int),
            'Volume': tendencia_diaria['Volume'].fillna(0).astype(int),
            'Premium ($)': tendencia_diaria['Daily_Premium'].round(2)
        })
        importar_o_actualizar_historial(df_para_db)
    ...
    analisis_ia_html, resumen_noticias_html = obtener_analisis_ia(ranking, ganador, stock_data, datos_dark, datos_lit, info_fund)
    
    # Construir sección de enlaces de noticias
    links_html = ""
    noticias_arr = info_fund.get('noticias', [])
    if noticias_arr:
        links_html = "<ul style='margin: 0; padding-left: 20px;'>"
        for n in noticias_arr:
            links_html += f"<li style='margin-bottom: 6px;'><a href='{n['link']}' target='_blank' style='color: #60a5fa; text-decoration: none; font-weight: 500;'>{n['title']}</a> <span style='color: #9ca3af; font-size: 8.5pt;'>({n['publisher']})</span></li>"
        links_html += "</ul>"
    else:
        links_html = "<p style='color: #9ca3af; margin: 0;'>No hay enlaces de noticias disponibles para este ticker.</p>"

    # Generar HTML seguro para el logo corporativo
    logo_url = info_fund.get('logo_url', '')
    logo_html = f"<img src='{logo_url}' style='width: 36px; height: 36px; vertical-align: middle; margin-right: 12px; border-radius: 6px; background: #ffffff; padding: 2px;' onerror='this.style.display=\"none\"'>" if logo_url else ""
    ...
    <head>
    <meta charset="UTF-8">
    <title>Informe Cuantitativo Integral con IA y Síntesis de Noticias</title>
    <style>
        /* IMPORTANTE: Las llaves en el CSS dentro de una f-string van dobles {{ }} */
        body {{ 
    ...
    <div class="container">
        <div class="header-banner">
            <h1>Informe Cuantitativo Integral con IA y Síntesis de Noticias</h1>
            <p>Estrategia TV (EMA 9/21, Volumen, MACD), Muros y Contexto de Mercado (Ticker: {ticker_ganador})</p>
        </div>

        <h2>Radiografía Fundamental y Perfil Corporativo</h2>
        <div class="card" style="background-color: #1f2937; border: 1px solid #374151; color: #f3f4f6;">
            <div style="display: flex; align-items: center; margin-bottom: 12px;">
                {logo_html}
                <div>
                    <span style="color: #60a5fa; font-size: 13pt; font-weight: bold;">{info_fund.get('long_name')} ({ticker_ganador})</span>
                    <div style="color: #9ca3af; font-size: 9pt;">{info_fund.get('sector')} &bull; {info_fund.get('industry')}</div>
                </div>
            </div>
            <p style="font-size: 9.5pt; color: #d1d5db; line-height: 1.4; margin-bottom: 12px;">
                <strong>¿Qué hace la empresa?</strong> {info_fund.get('summary')}
            </p>
            <hr style="border: 0; border-top: 1px solid #374151; margin: 12px 0;">
            <table style="width: 100%; font-size: 9.5pt; margin: 0; background: transparent;">
                <tr>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Capitalización:</strong> <span style="color: #f3f4f6;">{info_fund.get('market_cap')}</span></td>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Próximo Earnings:</strong> <span style="color: #f3f4f6;">{info_fund.get('proximo_earnings')}</span></td>
                </tr>
                <tr>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Ratio P/E (PER):</strong> <span style="color: #60a5fa; font-weight: bold;">{info_fund.get('pe')}</span></td>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Beta (Volatilidad):</strong> <span style="color: #60a5fa; font-weight: bold;">{info_fund.get('beta')}</span></td>
                </tr>
                <tr>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Rango 52 Semanas:</strong> <span style="color: #f3f4f6;">{info_fund.get('low_52')} – {info_fund.get('high_52')}</span></td>
                    <td style="border: none; padding: 4px 0; color: #9ca3af;"><strong>Dividend Yield:</strong> <span style="color: #f3f4f6;">{info_fund.get('dividend')}</span></td>
                </tr>
            </table>
        </div>
        ...
        {resumen_noticias_html}

        <h2>Enlaces para Profundizar en Noticias</h2>
        <div class="card">
            {links_html}
        </div>

        <h2>Contrato Ganador Detectado</h2>
        <div class="card" style="background-color: #1f2937; border: 1px solid #374151; border-left: 4px solid #3b82f6; padding: 15px 20px; color: #f3f4f6;">
            <p style="margin-top: 0; margin-bottom: 6px; color: #f3f4f6;"><strong>Opción Líder:</strong> {cp_ganador} con Strike <strong style="color: #60a5fa;">${strike_ganador}</strong> y Vencimiento <span style="color: #f3f4f6;">{exp_ganadora.strftime("%Y-%m-%d")}</span></p>'
            <p style="margin-bottom: 0; color: #f3f4f6;">Sentimiento institucional: <span style="color: #60a5fa; font-weight: bold;">{ganador['Sentiment_Dominante']}</span> | Puntuación Global: <span style="color: #60a5fa; font-weight: bold;">{ganador['Score']:.2f} / 100</span></p>
        </div>
    ...
    html_content += f"""
        ...
        <div class="footer">
            Informe generado con tu estrategia de TradingView, Síntesis de Noticias y Google Gemini IA &bull; Referencia: {fecha_analisis.strftime('%Y-%m-%d')}
        </div>
    </div>
    </body>
    </html>
    ...
    print("\nAbriendo el informe interactivo con síntesis de noticias y enlaces en tu navegador...")
